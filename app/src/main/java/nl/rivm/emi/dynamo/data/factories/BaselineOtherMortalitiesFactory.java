 ...
 * root-tagname.
 */
public class BaselineOtherMortalitiesFactory extends AgnosticFactory {
	...
	public TypedHashMap<Age> manufactureObservable(File configurationFile,
		...
		TypedHashMap<Age> producedMap = manufacture(configurationFile, true,
				rootElementName);
		BaselineOtherMortalitiesObject result = new BaselineOtherMortalitiesObject(producedMap);
		return (result);
	}
	...
	private BaselineOtherMortalitiesObject manufactureDefault(boolean makeObservable)
			throws ConfigurationException {
		log.debug("Starting manufacture.");
