 ...
 * root-tagname.
 */
public class AlphasOtherMortalityFactory extends AgnosticFactory {
	...
	public TypedHashMap<Age> manufactureObservable(File configurationFile,
		...
		TypedHashMap<Age> producedMap = manufacture(configurationFile, true,
				rootElementName);
		AlphasOtherMortalityObject result = new AlphasOtherMortalityObject(producedMap);
		return (result);
	}
	...
	private AlphasOtherMortalityObject manufactureDefault(boolean makeObservable)
			throws ConfigurationException {
		log.debug("Starting manufacture.");
