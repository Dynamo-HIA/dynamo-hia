...
public class ContinuousRiskFactorManyToOneUpdateRule extends
		DynamoManyToOneUpdateRuleBase implements ConfigurationEntryPoint {

	Log log = LogFactory
			.getLog("nl.rivm.emi.cdm.rules.update.dynamo.CategoricalRiskFactorManyToOneUpdateRule");
	...
	public ContinuousRiskFactorManyToOneUpdateRule()
	...
	public ContinuousRiskFactorManyToOneUpdateRule(String configFileName)
			throws ConfigurationException, CDMUpdateRuleException {
		// constructor fills the parameters
		...
		if (!success)
			throw new ConfigurationException(
					"loading of configuration file failed for updateRule CategoricalRiskFactorManyToOneUpdateRule");
	...
	public Object update(Object[] currentValues, Long seed)
		...
		try {
				...
				if (ageValue < 0) {
					...
					if (isNormal) {
						newValue = (float) (oldValue
								+ meanDrift[ageValue][sexValue] + stdDrift[ageValue][sexValue]
								* normInverse(pRandom));
					} else {
						double newOnLogScale = 0;
						if (oldValue > offset[ageValue][sexValue]){
							newOnLogScale = Math.log(oldValue
									- offset[ageValue][sexValue])
									+ meanDrift[ageValue][sexValue]
									+ stdDrift[ageValue][sexValue]
									* normInverse(pRandom);
						
						    newValue = (float) (Math.exp(newOnLogScale)
								+ offset[ageValue][sexValue] + offsetDrift[ageValue][sexValue]);}
						/* this is an at hoc solution for a situation that should  occur only very rarely */
						else newValue=offset[ageValue][sexValue]+offsetDrift[ageValue][sexValue];
			...
			log
					.fatal("this message was issued by ContinuousRiskFactorManyToOneUpdateRule"
	...
	configurationFileConfiguration.load(configurationFile) ;
			...
			if (!isNullTransitions) {
				...
				setMeanDrift(loadData(meanDriftFileName, "meandrift",
						"meandrift"));
				 ...
				 * 
				 * setMeanDrift(loadData(stdDriftFileName, "stddrift",
				 * "stddrift")); if (!isNormal) {
				 * 
				 * 
				 * } setAimValues();
				 */
