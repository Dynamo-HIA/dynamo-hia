...
public class CSVWriter  {
	...
	/**
	 * method writeOutput writes CSV files
	 * The following files are produced: depending on user input
	 * - separate workbooks for for
	...
	public void writeWorkBookCSVbyYear(String fileName, int sex, int thisScen)
		   ...
		   writer.append("year"+ this.delimiter);
			...
			if (this.details) {
				...
				for (int col = 4; col < this.output.getNDiseaseStates() + 3; col++) {
					writer.append( this.delimiter+ this.output.getStateNames()[col - 4]);
					// TODO: goede naam laten printen

				}
			} else {
				for (int col = 4; col < this.output.nDiseases + 4; col++) {
					writer.append(  this.delimiter+this.output.diseaseNames[col - 4]);
					// TODO: goede naam laten printen
			...
			for (int rClass = 0; rClass < this.output.nRiskFactorClasses; rClass++)
				...
				for (int a = 0; a < 96; a++) {
					...
					if (this.output.riskType == 1 || this.output.riskType == 3
						...
						writer.append( this.output.riskClassnames[rClass]+ this.delimiter);
					...
					if (this.details) {
						...
						for (int col = 4; col < this.output.nDiseaseStates + 3; col++) {
							...
							if (col==this.output.nDiseases + 3) writer.append( ((Double)data).toString());
							else writer.append( data+this.delimiter);
						}
		...
		catch (IOException e){ throw new DynamoOutputException("file " + fileName
	 ...
	 * @throws DynamoOutputException: when file can not be written
	 * @throws FileNotFoundException 
	
	
	 */
	public void writeWorkBookXMLbyCohort(String fileName, int sex, int thisScen) throws DynamoOutputException, FileNotFoundException
		...
		writer.append("age in startyear" + this.delimiter);
			...
			for (int cohort = 0; cohort < 96; cohort++) {
				...
				writer.append( ((Integer)(cohort +  this.output.startYear)).toString());
			...
			for (int rClass = 0; rClass < this.output.nRiskFactorClasses; rClass++)
				...
				for (int year = 0; year < this.output.nDim - cohort - 1; year++) {
					...
					if (this.output.riskType == 1 || this.output.riskType == 3
						...
						writer.append( this.delimiter+ this.output.riskClassnames[rClass]);
					...
					writer.append( this.delimiter+ this.output.startYear + year);
