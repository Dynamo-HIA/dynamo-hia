...
public class NestedTabSelectionListener implements SelectionListener {
	...
	public void widgetSelected(SelectionEvent e) {
		...
		log.debug("Selected item = "
				+ (myTabPlatform.getTabFolder().getSelection() == null ? "null" : myTabPlatform.getTabFolder()
						.getSelection()[0].toString()));
		...
		if (selected > -1) {
			...
			try {
				...
				log.debug("nestedTabs.size()" + myTabPlatform.getNestedTabs().size());
				log.debug("nestedTabs" + myTabPlatform.getNestedTabs());
				myTabPlatform.refreshNestedTab(myTabPlatform.getNestedTabs().get(item.getText()));
				...
				NestedTab removedNestedTab = myTabPlatform.getNestedTabs().get(tabItem
						.getText());
				// Remove the data from the data object model
				try {
					myTabPlatform.deleteNestedTab(removedNestedTab);
	...
	private void handleErrorMessage(Exception e) {
		e.printStackTrace();
		MessageBox box = new MessageBox(myTabPlatform.getTabFolder().getShell(),
