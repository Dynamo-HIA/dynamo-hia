...
public class ModelUpdateValueStrategies {
	...
	static Log log = LogFactory.getLog("nl.rivm.emi.dynamo.databinding.converters.ModelUpdateValueStrategies");
	...
	public static UpdateValueStrategy getStrategy(Object objectType) {
		...
		if (objectType instanceof Integer) {
			log.debug("Constructing modelvalueupdatestrategy for " + objectType.getClass().getName());
			resultStrategy = assembleIntegerModelValueUpdateStrategy(objectType);
		} else {
			log.error("No modelvalueupdatestrategy found for " + objectType.getClass().getName());
		}
		return resultStrategy;
	}
	
	private static UpdateValueStrategy assembleIntegerModelValueUpdateStrategy(Object type) {
		UpdateValueStrategy integerUpdateValueStrategy = new UpdateValueStrategy();
		integerUpdateValueStrategy.setConverter(ModelConverters.getConverter(type));	
