...
public abstract class AbstractFloatType extends NumberRangeTypeBase<Float> implements
		IXMLHandlingLayer<Float> {
	 ...
	 */
	static final public Pattern matchPattern = Pattern.compile("^\\d*,??\\d*$");

	public AbstractFloatType(String elementName, Float minValue, Float maxValue) throws ConfigurationException {
	...
	@Override
	Float convert4Model(String viewString) {
		Float result = null;
		Matcher matcher = matchPattern.matcher(viewString);
		if (matcher.matches()) {
			result = Float.parseFloat(viewString);
		} else {
		}
		return result;
	}
		...
		if (getXMLElementName().equals(node.getName())) {
			...
			if (valueObject != null) {
				if (valueObject instanceof String) {
					...
					Float convertedString = convert4Model(valueString);
