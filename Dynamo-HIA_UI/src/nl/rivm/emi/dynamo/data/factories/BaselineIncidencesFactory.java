...
import nl.rivm.emi.dynamo.data.objects.BaselineFatalIncidencesObject;
import nl.rivm.emi.dynamo.data.objects.BaselineIncidencesObject;
 ...
 * root-tagname.
 */
public class BaselineIncidencesFactory extends AgnosticFactory {
	...
	public TypedHashMap<Age> manufactureObservable(File configurationFile,
		...
		TypedHashMap<Age> producedMap = manufacture(configurationFile, true,
				rootElementName);
		BaselineIncidencesObject result = new BaselineIncidencesObject(producedMap);
		return (result);
	}
	...
	private BaselineIncidencesObject manufactureDefault(boolean makeObservable)
			throws ConfigurationException {
		log.debug("Starting manufacture.");
