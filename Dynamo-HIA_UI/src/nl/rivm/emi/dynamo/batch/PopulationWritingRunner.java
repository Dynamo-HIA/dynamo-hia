...
public class PopulationWritingRunner {
	...
	static private PopulationWritingRunner runner = null;

	public void run() {
		PopulationWritingRunner.main(new String[] { "arg", "arg" });
	}
		...
		if (batchFile.exists()) {
			if (batchFile.isFile()) {
				if (batchFile.canRead()) {
					runner = new PopulationWritingRunner();
