...
import nl.rivm.emi.dynamo.data.objects.RelativeRisksObject;
 ...
 */
public class RelativeRisksFactory extends AgnosticFactory {
	...
	public TypedHashMap<Age> manufactureObservable(File configurationFile,
		...
		TypedHashMap<Age> producedMap = manufacture(configurationFile, true,
				rootElementName);
		RelativeRisksObject result = new 	RelativeRisksObject(producedMap);
		return (result);
	}
	...
	private 	RelativeRisksObject manufactureDefault(boolean makeObservable)
			throws ConfigurationException {
		log.debug("Starting manufacture.");
