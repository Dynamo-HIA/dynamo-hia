 ...
 * root-tagname.
 */
public class BaselineFatalIncidencesFactory extends AgnosticFactory {
	...
	public TypedHashMap<Age> manufactureObservable(File configurationFile,
		...
		TypedHashMap<Age> producedMap = manufacture(configurationFile, true,
				rootElementName);
		BaselineFatalIncidencesObject result = new BaselineFatalIncidencesObject(producedMap);
		return (result);
	}
	...
	private BaselineFatalIncidencesObject manufactureDefault(boolean makeObservable)
			throws ConfigurationException {
		log.debug("Starting manufacture.");
