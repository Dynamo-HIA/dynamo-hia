...
import nl.rivm.emi.dynamo.data.objects.BaselineAbilitiesObject;
import nl.rivm.emi.dynamo.data.objects.BaselineOtherMortalitiesObject;
 ...
 * root-tagname.
 */
public class BaselineAbilitiesFactory extends AgnosticFactory {
	...
	public TypedHashMap<Age> manufactureObservable(File configurationFile,
		...
		TypedHashMap<Age> producedMap = manufacture(configurationFile, true,
				rootElementName);
		BaselineAbilitiesObject result = new BaselineAbilitiesObject(producedMap);
		return (result);
	}
	...
	private BaselineAbilitiesObject manufactureDefault(boolean makeObservable)
			throws ConfigurationException {
		log.debug("Starting manufacture.");
