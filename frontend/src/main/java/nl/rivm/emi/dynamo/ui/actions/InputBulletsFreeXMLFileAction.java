...
public class InputBulletsFreeXMLFileAction extends FreeNameXMLFileAction {
	...
	protected String getNewFilePath() {
		...
		TransitionTrialog inputDialog = new TransitionTrialog(shell,
				"BasePath: " + selectionPath,
				"Enter name for a new transition file", "Name",
				new FileAndDirectoryNameInputValidator(), this.riskFactorName,
				this.riskFactorType, rootElementName);
		...
		try {
			...
			if (RootElementNamesEnum.TRANSITIONDRIFT.getNodeLabel().equals(
				...
				if (BulletButtonNamesEnum.TRANSITION_USER_SPECIFIED
						.getBulletButtonName().equals(this.bulletButtonName)) {
					theModal = new TransitionDriftModal(shell, dataFile
							.getAbsolutePath(), savedFile.getAbsolutePath(),
							rootElementName, node);
				} else {
					if (BulletButtonNamesEnum.TRANSITION_NETTO
							.getBulletButtonName()
							.equals(this.bulletButtonName)) {
						theModal = new TransitionDriftNettoModal(shell,
								dataFile.getAbsolutePath(), savedFile
										.getAbsolutePath(),
								this.bulletButtonName, node);
					} else {
						if (BulletButtonNamesEnum.TRANSITION_ZERO
							...
							messageLineSet
									.add("That kind of file contains no data, so editing it is not possible.");
							messageLineSet
									.add("Click on \"Save\" to actually create the file.");
							theModal = new DataLessMessageModal(shell, dataFile
									.getAbsolutePath(), savedFile
									.getAbsolutePath(), rootElementName, node,
									caption, messageLineSet);
						} else {
				...
				if (RootElementNamesEnum.TRANSITIONMATRIX.getNodeLabel()
						.equals(rootElementName)) {
					if (BulletButtonNamesEnum.TRANSITION_USER_SPECIFIED
							.getBulletButtonName()
							.equals(this.bulletButtonName)) {
						theModal = new TransitionMatrixModal(shell, dataFile
								.getAbsolutePath(),
						...
						if (BulletButtonNamesEnum.TRANSITION_NETTO
							...
							if (BulletButtonNamesEnum.TRANSITION_ZERO
								...
								messageLineSet
										.add("Click on \"Save\" to actually create the file.");
								theModal = new DataLessMessageModal(shell,
										dataFile.getAbsolutePath(), savedFile
												.getAbsolutePath(),
										rootElementName, node, caption,
										messageLineSet);
							} else {
								throw new DynamoConfigurationException(
										"RadioButton " + bulletButtonName
