...
public class Output_ChangeScenarioTab {
	...
	public Output_ChangeScenarioTab(TabFolder tabfolder,CDMOutputFactory outputFactory,
	...
	public void makeIt() {
		...
		for (int i = 0; i < output.getNScen(); i++) {
			...
			slider1[i].setSelection((int) ( 10*params.getSuccesPercentage()[i]));
			...
			value1[i].setText(((Double) params.getSuccesPercentage()[i]).toString());
			...
			slider1[i].addSelectionListener(new SelectionListener() {
				public void widgetSelected(SelectionEvent event) {
					...
					params.setSuccesPercentage((value/10F), currenti);
					value1[currenti]
							.setText(((Double) params.getSuccesPercentage()[currenti])
									.toString());
			...
			slider2[i].setSelection((int) params.getMinAge()[i]);
		...
		//	value2[i].setDigits(0);
			...
			value2[i].setText(((Integer)  params.getMinAge()[i]).toString());
			...
			slider2[i].addSelectionListener(new SelectionListener() {
				public void widgetSelected(SelectionEvent event) {
					...
					params.setMinAge(value, currenti);
					value2[currenti]
							.setText(((Integer) params.getMinAge()[currenti])
									.toString());
			...
			slider3[i].setSelection((int) this.params.getMaxAge()[i]);
			...
			value3[i].setText(((Integer) this.params.getMaxAge()[i]).toString());
			...
			slider3[i].addSelectionListener(new SelectionListener() {
				public void widgetSelected(SelectionEvent event) {
					...
					params.setMaxAge(value, currenti);
					value3[currenti]
							.setText(((Integer) params.getMaxAge()[currenti])
									.toString());
			...
			if (params.getInMen()[i] && params.getInWomen()[i])
				combo[i].select(2);
			else if (params.getInMen()[i])
				combo[i].select(0);
			else if (params.getInWomen()[i])
				combo[i].select(1);
			...
			combo[i].addSelectionListener(new SelectionAdapter() {
				...
				public void widgetSelected(SelectionEvent e) {
					...
					for (int i = 0; i < combo.length; i++) {
						...
						switch (choice) {
						case 0:
							params.setInMen(i, true);
							params.setInWomen(i, false);
							break;

						case 1:
							params.setInMen(i, false);
							params.setInWomen(i, true);
							break;
						case 2:
							params.setInMen(i, true);
							params.setInWomen(i, true);
