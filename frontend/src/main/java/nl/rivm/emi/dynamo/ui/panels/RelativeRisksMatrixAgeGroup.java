...
public class RelativeRisksMatrixAgeGroup extends Composite {
	...
	RelativeRisksMatrixOneAgeAndSexParameterGroup[] relRisksMatGroups;
	int currentAge = 0;
	boolean grow = false;
	private Point initialSize;

	@SuppressWarnings("unchecked")
	public RelativeRisksMatrixAgeGroup(Shell parent, int style,
		...
		relRisksMatGroups = new RelativeRisksMatrixOneAgeAndSexParameterGroup[maxGender
				- minGender + 1];
		refreshTransitionMatrices(currentAge, ageDropDownPanel);
		parent.pack(true);
		initialSize = relRisksMatGroups[0].theGroup.getSize();
	}
	...
	public void refreshTransitionMatrices(int age, Group topNeighbour) {
		...
		for (int genderCount = minGender; genderCount <= maxGender; genderCount++) {
			refreshRelativeRisksMatrixGroup(this, age, genderCount, topNeighbour);
		}
		layoutRefreshedRelativeRisksMatrixGroups(this, age, topNeighbour);
	...
	@SuppressWarnings("unchecked")
	private void refreshRelativeRisksMatrixGroup(Composite parent, int age,
			int gender, Composite topNeighbour) {
		// remove if present.
		if (relRisksMatGroups[gender] != null) {
			relRisksMatGroups[gender].theGroup.dispose();
		}
		relRisksMatGroups[gender] = new RelativeRisksMatrixOneAgeAndSexParameterGroup(
				parent,
	...
	private void layoutRefreshedRelativeRisksMatrixGroups(Composite parent,
		...
		femaleLayoutData.right = new FormAttachment(relRisksMatGroups[1]
				.getGroup(), -2);
		femaleLayoutData.bottom = new FormAttachment(100, -2);
		relRisksMatGroups[0].getGroup().setLayoutData(femaleLayoutData);
		FormData maleLayoutData = new FormData();
		maleLayoutData.top = new FormAttachment(topNeighbour, 2);
		maleLayoutData.left = new FormAttachment(relRisksMatGroups[0].getGroup(),
				2);
		maleLayoutData.right = new FormAttachment(100, -2);
		maleLayoutData.bottom = new FormAttachment(100, -2);
		relRisksMatGroups[1].getGroup().setLayoutData(maleLayoutData);
	}
